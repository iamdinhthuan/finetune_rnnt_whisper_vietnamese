...
class AudioDataset(Dataset):
    def __init__(self,
        ...
        if augment:
            # Tạo augmentation riêng cho câu ngắn
            self.short_sentence_augmentation = Compose([
                Gain(min_gain_db=-15, max_gain_db=15, p=0.95),  # Tăng gain để tăng cường tín hiệu
                TimeStretch(min_rate=0.95, max_rate=1.05, p=0.3),  # Stretch ít hơn
                PitchShift(min_semitones=-2, max_semitones=2, p=0.3),  # Pitch shift nhẹ hơn
            ])

            # Augmentation thông thường
            self.augmentation = Compose(
                [
                    ...
                    OneOf(
                        [
                            AddBackgroundNoise(sounds_path=bg_noise_path, min_snr_db=1.0, max_snr_db=5.0,
                                               noise_transform=PolarityInversion(), p=0.9),
            ...
            self.short_sentence_augmentation = lambda samples, sample_rate: samples
    ...
    def __getitem__(self, idx):
        ...
        # Áp dụng augmentation khác nhau cho câu ngắn
        if len(sample['text'].strip()) <= 5:  # Nếu câu ngắn (ít hơn 5 ký tự)
            waveform = self.short_sentence_augmentation(samples=waveform, sample_rate=sample_rate)
        else:
